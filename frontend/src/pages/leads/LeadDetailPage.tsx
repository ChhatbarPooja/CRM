...
export default function LeadDetailPage() {
  ...
  const handleEditSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      const wasAssigneeChanged = 
        currentLead?.assignedTo?._id !== editForm.assignedTo && 
        editForm.assignedTo !== '';
      ...
      // Show additional toast for assignment
      if (wasAssigneeChanged) {
        const assignee = salesReps.find((rep: any) => rep._id === editForm.assignedTo);
        if (assignee) {
  ...
  const handleActivitySubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      // Show specific toast message based on activity type
      if (activityForm.type === 'follow-up' || activityForm.type === 'meeting') {
        if (currentLead?.assignedTo) {
          toast.success(`Activity scheduled and notification sent to ${currentLead.assignedTo.name}`);
        } else {
