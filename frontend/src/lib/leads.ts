...
export const useLeadsStore = create<LeadsState>((set, get) => ({
  ...
  updateLead: async (id: string, leadData: Partial<LeadFormValues>) => {
    try {
      ...
      // Check if we're changing the assignee to track for notification
      const { currentLead } = get();
      const isAssigneeChanged = currentLead && 
        leadData.assignedTo !== undefined && 
        currentLead.assignedTo?._id !== leadData.assignedTo;
  ...
  addActivity: async (leadId: string, activityData: any) => {
    try {
      ...
      // Include notification flag for activities that require notification
      if (activityData.type === 'follow-up' || activityData.type === 'meeting') {
        activityData.sendNotification = true;
      }
